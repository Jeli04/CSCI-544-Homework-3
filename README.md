...
```bash
python train_blstm.py --config configs/blstm1.json
```
- Optimizer: SGD, lr=0.1, momentum=0.9
- Scheduler: ReduceLROnPlateau (factor=0.1)
- No pre-trained embeddings

Saves the best checkpoint to `blstm.pt`. Make sure to rename to `blstm1.pt`.
...
```bash
python train_blstm.py --config configs/blstm2.json
```
- Optimizer: Adam, lr=0.001
- Scheduler: ReduceLROnPlateau (factor=0.5)
- Pre-trained GloVe embeddings (glove.6B.100d)
- Case embedding (dim=5) for capitalization features

Saves the best checkpoint to `blstm.pt`. Make sure to rename to `blstm2.pt`.
...
```bash
python train_cnn_blstm.py --config configs/cnn_blstm.json
```
...
```bash
python inference.py --checkpoint blstm1.pt --config configs/blstm1.json
```

Run inference for Task 2 (blstm2):
```bash
python inference.py --checkpoint blstm2.pt --config configs/blstm2.json
```

Run inference for Bonus (CNN-BiLSTM-CRF):
```bash
python inference.py --checkpoint blstm_cnn.pt --config configs/cnn_blstm.json --model cnn_blstm
```
